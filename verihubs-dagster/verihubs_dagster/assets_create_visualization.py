...
def create_daily_visualization(extract_csv: pd.DataFrame):
    """Create chart to observe daily order separated by status."""
    
    # Transform dataframe based on visualization needs
    df_daily = sql.daily_orders_by_status_df(extract_csv)
...
def create_monthly_visualization(extract_csv: pd.DataFrame):
    """Create card that shows the month with highest total revenue."""
    
    # Transform dataframe based on visualization needs
    df_monthly = sql.monthly_highest_revenue(extract_csv)
    ...
    plt.text(x = 0.5,
            y = 0.3,
            s='{:,}'.format(df_monthly['total_revenue'].loc[df_monthly.index[0]]),
            fontdict = {'fontsize': 25},
            horizontalalignment = 'center')
    plt.xticks([])
    plt.yticks([])
